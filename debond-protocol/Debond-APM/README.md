...
[APM.sol](./contracts/APM.sol) consist of the following  mappings:

```solidity
    mapping(address => uint256) internal totalReserve; // mapping of (Bond Tokenised Address => supply of token overall ).
    mapping(address => uint256) internal totalEntries; (mapping of the total volume of liquidity pair)
    mapping(address => mapping( address => uint)) entries; // mapping of the token address => bond address => value.
```
...
3. Then set the corresponding address of dependency `APMRouter`,(ie DebondToken,  `Bank`, `Bond` and `Governance`) using the methods of governanceOwnable dependencies. 
...
```bash
> npm install @Debond/APM
```

2. integrating the APM pool to the contract whose address is set as 'bankAddress' by importing the 'IAPM' address.
